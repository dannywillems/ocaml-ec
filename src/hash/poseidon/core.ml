...
module type VARIABLE_INPUT_LENGTH_PARAMETERS = PARAMETERS
...
module MakeStrategy (C : PARAMETERS) (Scalar : Ff_sig.PRIME) = struct
  open C
...
module MakeVariableInputLength (C : PARAMETERS) (Scalar : Ff_sig.PRIME) = struct
  open C
  ...
  module Strategy = MakeStrategy (C) (Scalar)
...
struct
  ...
  module Hash = struct
    ...
    let digest state data =
      let l = Array.length data in
      let chunk_size = width - 1 in
      let nb_full_chunk = (l - 1) / chunk_size in
      (* we process first all the full chunks *)
      for i = 0 to nb_full_chunk - 1 do
        let ofs = i * chunk_size in
        for j = 0 to chunk_size - 1 do
      ...
      (* we add the last partial chunk, add pad with one *)
      let r = l - (nb_full_chunk * (width - 1)) in
      for j = 0 to r - 1 do
        let idx = 1 + j in
        Strategy.add_cst state idx data.((nb_full_chunk * chunk_size) + j)
      done ;
