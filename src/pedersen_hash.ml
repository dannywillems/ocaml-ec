  ...
  let hash iterator =
      ...
      if j = Params.chunks_per_generator + 1 then (acc, false)
        ...
        *)
        if Option.is_none a_opt then (acc, true)
          ...
          let power =
            Ec.ScalarField.pow
              Ec.ScalarField.(one + one)
              (Z.of_int (4 * (j - 1)))
          in
          let acc = Ec.ScalarField.add acc (Ec.ScalarField.mul enc_m_j power) in
          compute_m_j acc (j + 1)
    in
    let rec compute_segment_i_incremental acc_res generators =
      ...
      | generator :: generators ->
          let (sum_m_j, fully_processed) = compute_m_j Ec.ScalarField.zero 1 in
          (* compute segment i *)
