...
let test_generation () =
  ...
  let pt_mt1 = from_twisted_to_montgomery pt_ed1 in
  assert (Option.(is_some pt_mt1)) ;
  let pt_wt1 = from_twisted_to_weierstrass pt_ed1 in
  assert (Option.(is_some pt_wt1)) ;
  let pt_mt2 = AffineMontgomery.random () in
  let pt_wt2 = from_montgomery_to_weierstrass pt_mt2 in
  assert (Option.(is_some pt_wt2)) ;
  let pt_ed2 = from_montgomery_to_twisted pt_mt2 in
...
let test_addition () =
  ...
  (* Twisted -> Montgomery *)
  let pt_mt1 = from_twisted_to_montgomery pt_ed1 |> Option.get in
  let pt_mt2 = from_twisted_to_montgomery pt_ed2 |> Option.get in
  let pt_mt1_add = AffineMontgomery.add pt_mt1 pt_mt2 in
  let pt_ed2_add = from_montgomery_to_twisted pt_mt1_add |> Option.get in
  let pt_mt2_add = from_twisted_to_montgomery pt_ed1_add |> Option.get in
  assert (AffineMontgomery.eq pt_mt1_add pt_mt2_add) ;
  assert (AffineEdwards.eq pt_ed1_add pt_ed2_add) ;
  (* Twisted -> Weierstrass *)
  let pt_wt1 = from_twisted_to_weierstrass pt_ed1 |> Option.get in
  let pt_wt2 = from_twisted_to_weierstrass pt_ed2 |> Option.get in
  let pt_wt1_add = AffineWeierstrass.add pt_wt1 pt_wt2 in
  let pt_wt2_add = from_twisted_to_weierstrass pt_ed1_add |> Option.get in
  ...
  let pt_eda = from_montgomery_to_twisted pt_mta |> Option.get in
  let pt_edb = from_montgomery_to_twisted pt_mtb |> Option.get in
  let pt_eda_add = AffineEdwards.add pt_eda pt_edb in
  let pt_mtb_add = from_twisted_to_montgomery pt_eda_add |> Option.get in
  let pt_edb_add = from_montgomery_to_twisted pt_mta_add |> Option.get in
...
let test_multiplication () =
  ...
  let pt_ed2_mul = from_montgomery_to_twisted pt_mt1_mul |> Option.get in
  let pt_mt2_mul = from_twisted_to_montgomery pt_ed1_mul |> Option.get in
  ...
  let pt_wt2_mul = from_twisted_to_weierstrass pt_ed1_mul |> Option.get in
  ...
  let pt_mtb_mul = from_twisted_to_montgomery pt_eda_mul |> Option.get in
  let pt_edb_mul = from_montgomery_to_twisted pt_mta_mul |> Option.get in
  assert (AffineEdwards.eq pt_eda_mul pt_edb_mul) ;
  assert (AffineMontgomery.eq pt_mta_mul pt_mtb_mul)

let get_tests () =
  let open Alcotest in
  ( "Twisted <-> Montgomery ; Twisted -> Weierstrass",
